...
func (m *Manager) Destroy(ctx context.Context, id string) error {
	...
	stopErr := m.docker.ContainerStop(ctx, containerName(id), container.StopOptions{Timeout: dockerutil.IntPtr(10)})
...
func (m *Manager) RecoverOrphans(ctx context.Context) error {
	...
	for _, c := range stuck {
		...
		_ = m.docker.ContainerStop(ctx, containerName(c.ID), container.StopOptions{Timeout: dockerutil.IntPtr(5)})
	...
	for _, ct := range containerList {
		...
		if _, err := m.copies.Get(copyID); err != nil {
			...
			_ = m.docker.ContainerStop(ctx, ct.ID, container.StopOptions{Timeout: dockerutil.IntPtr(5)})
...
func (m *Manager) provision(ctx context.Context, opts CreateOptions, ttl int, warm bool) (*store.Copy, error) {
	...
	cleanup := func(cause error) {
		if containerStarted {
			_ = m.docker.ContainerStop(context.Background(), containerName(id), container.StopOptions{Timeout: dockerutil.IntPtr(10)})
