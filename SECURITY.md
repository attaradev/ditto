...
databases, keep `allow_destructive_refresh` disabled for any target that should not be cleaned, and
use admin-only shared-host tokens for `/v2/targets/{name}/refresh`.
